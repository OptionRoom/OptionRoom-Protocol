...
contract CourtFarming{
    ...
    uint256 private _accRewardPerToken; // accumulative rewared per token
    mapping(address => uint256) private _rewards; // rewareds balances
    mapping(address => uint256) private _prevAccRewardPerToken; // previous accumulative rewared per token (for a user)
    uint256 public _finishBlock; // finish rewarding block number
    
    
    // incentive rewards
    uint256 private _incvRewardPerBlock; // incentive reward per block
    uint256 private _incvAccRewardPerToken; // accumulative rewared per token
    mapping(address => uint256) private _incvRewards; // rewareds balances
    mapping(address => uint256) private _incvPrevAccRewardPerToken;// previous accumulative rewared per token (for a user)
    ...
    function setCourtStake(address courtStakeAdd) public{
        require(msg.sender == _owner, "only contract owne can change");
